...
const { nationCensusId } = require('./const/nations.js');
...
async function sortdata() {
  ...
  console.log(
    `The program ran from ${Starttime} and ended at ${Endtime}. A difference of ${diff} seconds, or ${Mindiff} minutes.`,
  );
}
...
for (let i = 0; i < countProperties(nationCensusId); i++) {
  for (let j = 1; j < nationCensusId[i].length; j++) {
    ...
    const prasexml = async () => {
      const websiteresult = await schedule(
        fetchCensusfunc,
        format,
        nationCensusId[i][0],
        format2,
      ...
      console.log(
        `${nationCensusId[i][0]}: Census: ${censuslist[nationCensusId[i][j]]}(id:${
          nationCensusId[i][j]
        }): ${stringify3}`,
